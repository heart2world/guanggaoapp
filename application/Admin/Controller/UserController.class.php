...
class UserController extends AdminbaseController{
	...
	public function index(){
		...
		$user_nicename= I('request.user_nicename');
		if($user_login){
        ...
        foreach ($users as $key => $value) {
        	$users[$key]['role_name'] = D()->table(C('DB_PREFIX')."role r,".C('DB_PREFIX')."role_user u")->where("r.id=u.role_id and u.user_id='".$value['id']."'")->getField('r.name');
        }
	...
	public function detail(){
		$id=intval($_GET['id']);
		$user=$this->users_model->where(array("id"=>$id))->find();
		$user['role_name'] =D()->table(C('DB_PREFIX')."role r,".C('DB_PREFIX')."role_user u")->where("r.id=u.role_id and u.user_id='".$user['id']."'")->getField('r.name');
		$this->assign($user);
		$this->display();
	}
	// 管理员个人信息修改提交
