...
class AdminbaseController extends AppframeController {
    ...
    //excel导出
    public function exportExcel($expTitle,$expCellName,$expTableData,$condition=false){
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth('30');//设置列宽
        ...
        //设置垂直、水平居中
        $objPHPExcel->getActiveSheet()->getStyle('A:N')->getAlignment()
        ...
        //销量表需要合并操作
        if ($condition) {
            $objPHPExcel->getActiveSheet()->mergeCells($condition['row']);
            $objPHPExcel->getActiveSheet(0)->setCellValue(substr($condition['row'],0,2),'合计');
            $objPHPExcel->getActiveSheet(0)->setCellValue($condition['row1'],$condition['price']);
            $objPHPExcel->getActiveSheet(0)->setCellValue($condition['row2'],$condition['profit']);
        }
